...
MODEL_COSTS: dict[tuple[str, str], ModelCost] = {
    ...
    ("gemini", "gemini-2.5-flash-lite"): ModelCost(
        "gemini", "gemini-2.5-flash-lite", 0.10, 0.40, note="Estimate; verify against Google billing."
    ),
    ("gemini", "gemini-2.5-flash"): ModelCost("gemini", "gemini-2.5-flash", 0.30, 2.50, note="Estimate; verify against Google billing."),
    ("gemini", "gemini-2.5-pro"): ModelCost(
        "gemini", "gemini-2.5-pro", 1.25, 10.00, note="Estimate for prompts up to 200k tokens; verify against Google billing."
    ),
    ("kimi", "kimi-k2.5"): ModelCost("kimi", "kimi-k2.5", 0.15, 2.50, note="Estimate; verify against provider billing."),
    ("kimi", "kimi-k2.6"): ModelCost(
        "kimi", "kimi-k2.6", 0.95, 4.00, note="Moonshot direct cache-miss estimate; cache-hit input may be cheaper."
    ),
    ...
    ("ernie", "ernie-5.1"): ModelCost(
        "ernie", "ernie-5.1", 0.0, 0.0, note="Qianfan API model; pricing must be verified in the Baidu console before production use."
    ),
}
...
def estimate_cost(provider: str, model: str, input_tokens: int, output_tokens: int = 0) -> dict[str, object]:
    ...
    amount = (input_tokens / 1_000_000 * cost.input_per_million) + (output_tokens / 1_000_000 * cost.output_per_million)
    return {
        "provider": provider,
        "model": model,
        "known": True,
